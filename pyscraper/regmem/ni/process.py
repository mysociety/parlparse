...
def fetch_data_remote():
    ...
    lru_cache is used to prevent multiple downloads.
    """
...
def create_ni_universal_for_date(
    ...
    """
    ni = GenericRegmemRegister(
        chamber=Chamber.NORTHERN_IRELAND, published_date=closing_date
    )

    entries.sort(key=lambda x: x.person_id)
    for person_id, person_entries in groupby(entries, key=lambda x: x.person_id):
        person_entries = list(entries)
        ...
        if person is None:
            ...
            continue
        person = GenericRegmemPerson(
            person_id=person.id, person_name=person_name, published_date=closing_date
        )
...
def create_register_file_for_date(
    ...
    dest_folder = (
        parldata_path / "scrapedjson" / "common_regmem" / Chamber.NORTHERN_IRELAND
    )
