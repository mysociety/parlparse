...
def get_ms_details(
    ms_id: int, *, lang: Literal["en", "cy"] = "en"
) -> GenericRegmemPerson:
    """
    ...
    categories: list[GenericRegmemCategory] = []
    ...
    for caption, table in zip_longest(captions, items):
        ...
        for item in table.fillna("None").to_dict(orient="records"):
            entry = GenericRegmemEntry(
                details=[
                    GenericRegmemDetail(name=str(k), value=v) for k, v in item.items()
                ]
            )
...
def get_current_register(
    ...
    dest_folder = parldata_path / "scrapedjson" / "common_regmem" / "senedd" / lang
