...
class LondonRegisters:
    ...
    def get_roi(self):
        ...
        for category_str, items in found_items.items():
            ...
            category = GenericRegmemCategory(
                ...
                category_id=cat_id,
                entries=[GenericRegmemEntry(description=x) for x in items],
            )
    ...
    def get_gifts(self) -> Optional[GenericRegmemCategory]:
        ...
        df = dfs[0]
        results: list[GenericRegmemEntry] = []

        for df in dfs:
            entries = df.to_dict(orient="records")

            for raw_entry in entries:
                entry = GenericRegmemEntry(
                    description=raw_entry["Details"],
                    date_received=datetime.datetime.strptime(
                        raw_entry["Date gifted"], "%d %B %Y"
                    ).date(),
                )
                entry.details = [
                    GenericRegmemDetail(name=str(x), value=y)
                    for x, y in raw_entry.items()
                    if x not in ["Details", "Date gifted"]
                ]
                results.append(entry)

        return GenericRegmemCategory(
...
def get_current_registry(quiet: bool = False):
    ...
    return GenericRegmemRegister(
        chamber=Chamber.LONDON, published_date=last_update_date, entries=all_people
    )
...
def write_register(force_refresh: bool = False, quiet: bool = False):
    ...
    for person in register.entries:
    ...
    dest_folder = parldata_path / "scrapedjson" / "common_regmem" / Chamber.LONDON
