...
def api_field_to_uni_detail(field: CommonsAPIFieldModel) -> GenericRegmemDetail:
    detail = GenericRegmemDetail(
        name=nice_name(field.name),
    ...
    for subitem_list in field.values or []:
        for item in subitem_list:
            detail.sub_items.append(api_field_to_uni_detail(item))

    return detail


def api_interest_to_uni_regmem_item(
    api_interest: CommonsAPIPublishedInterest,
) -> GenericRegmemEntry:
    uni_interest = GenericRegmemEntry(
        original_id=str(api_interest.id),
        description=api_interest.summary,
    ...
    for field in api_interest.fields:
        detail = api_field_to_uni_detail(field)
        uni_interest.details.append(detail)
        if detail.name == "Received Date":
            uni_interest.date_received = detail.value

    for child in api_interest.child_items:
        uni_interest.sub_items.append(api_interest_to_uni_regmem_item(child))
...
class RegisterManager:
    ...
    def get_universal_register(self, date: datetime.date) -> GenericRegmemRegister:
        """
        ...
        uni = GenericRegmemRegister(
            chamber=Chamber.COMMONS, published_date=date, entries=[]
        )
        ...
        for api_member in interests:
            ...
            for api_category in api_member.categories:
                ...
                for api_interest in api_category.interests:
                    uni_interest = api_interest_to_uni_regmem_item(api_interest)

                    uni_cat.entries.append(uni_interest)

                person_entry.categories.append(uni_cat)

            uni.entries.append(person_entry)
    ...
    def write_universal_regmem(self, force_refresh: bool = False, quiet: bool = False):
        ...
        dest_folder = self.parldata_dir / "scrapedjson" / "common_regmem" / "commons"
        ...
        if not dest.exists() or force_refresh:
            ...
            mysoc_register.to_path(dest_folder / filename)
            write_register_to_xml(
...
def convert_xml_folder():
    ...
    json_folder = parldata_path / "scrapedjson" / "common_regmem" / "commons"
