...
def compute_membership_reasons(
    ...
    Rules (per person + seat group, sorted by start_date):
      start_reason:
        - start_date is near cycle_start → ELECTION
        - Previous membership had a different party → CHANGED_PARTY
        - Otherwise → BLANK

      end_reason:
        - end_date is near cycle_end → DISSOLUTION
        - Next membership exists with a different party → CHANGED_PARTY
        - Last membership for this person+seat, ended early → RESIGNED
        - Otherwise → BLANK
    """

    def sort_by_person_and_seat(m: SPMembership) -> tuple[int, str]:
        return (m.scottish_parl_person_id, m.constituency_or_region_name)

    sorted_memberships = sorted(memberships, key=sort_by_person_and_seat)

    result: list[SPMembership] = []

    # iterate through all memberships for a person/seat
    for _, group_iter in groupby(sorted_memberships, key=sort_by_person_and_seat):
        ...
        for pos, m in enumerate(group):
            ...
            # end_date happens near dissolution
            if (
                m.end_date
                and m.cycle_end
