...
def add_sp_identifier(m: Membership, membership_id: int) -> None:
    ...
    if has_sp_identifier(m, membership_id):
        return
...
def sync_sp_memberships(sp_memberships: SPMembershipList, popolo: Popolo) -> list[str]:
    ...
    Existing identifiers
        Revalidate party and dates on every run; matching identifiers are not duplicated.
    ...
    for sp in sp_memberships:
        ...
        identified = [m for m in existing if has_sp_identifier(m, sp.membership_id)]
        overlapping = identified or [m for m in existing if membership_overlaps(sp, m)]
        ...
        if len(overlapping) == 0:
            ...
            if m.on_behalf_of_id != party_org_id:
                log.append(
                    f"MISMATCH (party): {m.id} ({person.id}) "
                    f"{m.on_behalf_of_id} vs API {party_org_id} — manual review needed "
                    f"(membership_id={sp.membership_id})"
                )
                continue

            already_identified = has_sp_identifier(m, sp.membership_id)
