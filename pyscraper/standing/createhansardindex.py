...
def GetReportProceedings(urlpage, year):
	...
	if year == "2008":
        ...
        if year == "2015":
                ...
                vdat = re.sub('<a href="[^"]*highspeedrail[^"]*">Committee: (1st|2nd|3rd) sitting</a>', '', vdat)
	...
	for lk in lks:
		...
		lkname = re.sub("(?:\s|&nbsp;|<br>|</?[iI]>)+", " ", lk[1]).strip()
                ...
                if int(year) >= 2009:
                        ...
                        msitting = re.match('(?:Public Bill )?Comm?itt?ee?(?: Stage| debates?)*(?: ?-|:)? *(\d+)(?:st|nd|rd|th) sitting(?i)', lkname)
			try:
                                sitting = int(msitting.group(1))
                        except:
                                raise Exception, "Could not find sitting in %s for %s" % (lkname, urlpage)
                        mdate = re.search('cmpublic/(.*?)/(\d\d)(\d\d)(\d\d)(?:/(am|pm))?/(?:pt(\d)/)?\d+s\d+(?:(?:part|p|pt)(\d))?\.htm', lklk)
