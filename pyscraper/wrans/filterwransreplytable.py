...
recolmatch = re.compile('<t[dh](?: class[ =]"tabletext")?(?: align=\"?(center|right)\"?)?(?: colspan=\"?(\d+)\"?)?(?: rowspan=\"?(\d+)\"?)?(?: align=\"?(center|right)\"?)?(?: valign=top)?(?: colspan=\"?(\d+)\"?)?(?: NOWRAP)?(?: class="tabletext")?>\s*([\s\S]*?)\s*(?:</t[dh]>)?$(?i)')
