...
def _restore_paragraph_breaks(text: str) -> str:
    """
    The Scottish Parliament API strips paragraph breaks from answer text,
    leaving sentences joined with no space (e.g. "...funding route.Additional
    awards...").  Detect these junctions and re-insert newlines so downstream
    paragraph splitting works.

    The pattern: a sentence-ending character (. ? ! ) ]) followed immediately
    by an uppercase letter starting a new word — with no whitespace between.
    """
    return re.sub(
        r"([.?!;)\]])"  # sentence-ending punctuation (including ; used as para break)
        r"(?=[A-Z][a-z])",  # followed by uppercase then lowercase (new sentence)
        r"\1\n\n",
        text,
    )
...
def _text_block_to_speech_items(
    ...
    paragraph boundaries (blank lines or restored breaks).
    """
    ...
    # Restore paragraph breaks stripped by the SP API
